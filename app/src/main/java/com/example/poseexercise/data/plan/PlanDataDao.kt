...
interface PlanDataDao {
    ...
    @Query("SELECT * FROM plan_items WHERE UPPER(selectedDays) LIKE '%' || UPPER(:day) || '%'")
    fun getPlansByDay(day: String?): List<Plan>

    /** Get not completed plans from day of the week */
    @Query("SELECT * FROM plan_items WHERE UPPER(selectedDays) LIKE '%' || UPPER(:day) || '%' AND completed = 0")
